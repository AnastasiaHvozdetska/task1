...
(showResults) = () => {
    ...
    for (var i in data.A) {
        data.A.sort((a, b) => parseFloat(a.sortBy) - parseFloat(b.sortBy));
        dataA += '<tr>' +
            '<td>' + (data.A[i].company_name || '-') + '</td>' +
            '<td>' + (data.A[i].product || '-') + '</td>' +
            '<td>' + (data.A[i].price || '-') + '</td>' +
            '<td>' + (data.A[i].fda_date_approved || '-') + '</td>' +
            '</tr>';
    }
 
    for (var i in data.B) {
        dataB += '<tr>' +
            '<td>' + (data.B[i].first_name || '-') + '</td>' +
            '<td>' + (data.B[i].last_name || '-') + '</td>' +
            '<td>' + (data.B[i].country || '-') + '</td>' +
            '<td>' + (data.B[i].phone || '-') + '</td>' +
            '</tr>';
    }
 
    document.querySelector('#A tbody').innerHTML = dataA;
    document.querySelector('#B tbody').innerHTML = dataB;
 }
 
 (sortBy) = (property) => {
    return function (a, b) {
        var result = 
            (typeof a[property] && b[property] === undefined) ? -1 : 
 ...
 document.querySelector('body').addEventListener('click', function (el) {
    const table_id = el.target.closest('table').id;
    const column_id = el.target.id;
 
    data[table_id].sort(sortBy(column_id));
    showResults();
 })
